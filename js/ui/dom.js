...
/* 通用對話框：modal({html, buttons:[{label, cls, value}]}) → Promise<value> */
let modalResolve = null;
export function modal(html, buttons=[], {dismiss=null}={}){
  const box=$('modal-box');
  box.innerHTML = html + (buttons.length?`<div class="flex flex-col gap-2 mt-4">${buttons.map((b,i)=>`<button class="tap rounded-xl py-3 font-bold ${b.cls||'btn-ghost'}" data-act="modal-btn" data-i="${i}">${b.label}</button>`).join('')}</div>`:'');
...
export const confirmBox = (title, text, ok='確定', cancel='取消', danger=false) =>
  modal(`<h2 class="text-lg font-black mb-2">${title}</h2><p class="text-sm text-gray-300 leading-relaxed">${text}</p>`,
...
/* 一般頁面：標題列（可返回）+ 可捲動內容 + 底部按鈕列 */
export function page({title, sub='', back=null, right='', body='', footer=''}){
  showScreen('view');
  $('screen-view').innerHTML = `
  <header class="shrink-0 px-3 pt-3 pb-2 flex items-center gap-2">
    ${back?`<button data-act="${back}" class="tap w-10 text-xl shrink-0" aria-label="返回">←</button>`:''}
    <div class="flex-1 min-w-0"><h1 class="text-lg font-black truncate">${title}</h1>${sub?`<div class="text-[11px] text-gray-400 truncate">${sub}</div>`:''}</div>
    ${right}
  </header>
