...
import { go, registerNav, monsterInfoHtml } from './menu.js';
...
const dropsHtml = drops => Object.entries(drops).map(([id,n])=>`<span class="inline-flex items-center gap-1 mr-2 mb-1 text-[11px]">${matIcon(id,22)}${matName(id)}${MATERIALS[id].kind==='rare'?'<span class="text-amber-300">★</span>':''} ×${n}</span>`).join('');
...
async function storyNew(){
  ...
  draft = {name:'獵人', icon:'🧑', weapon:'gs'};
  createScreen();
}
function createScreen(){
  page({title:'📖 建立獵人', sub:'劇情模式由一位獵人獨自挑戰', back:'go-menu', body:`
    <div class="panel p-3 mt-2">
      <div class="flex gap-2 mb-3">
        ...
        <label class="w-16 shrink-0"><span class="block text-[10px] text-gray-500 mb-0.5">頭像</span>
          <button data-act="new-icon" class="w-full bg-black/40 border border-gray-700 rounded-lg py-1.5 text-xl">${draft.icon}</button></label>
      </div>
      <div class="text-[10px] text-gray-500 mb-1">選擇武器（之後可在集會所隨時更換）</div>
      <div class="grid grid-cols-5 gap-1.5">${WEAPON_IDS.map(w=>`<button data-act="new-weapon" data-w="${w}" class="wpn-pick ${draft.weapon===w?'sel':''}"><div class="text-xl">${WEAPONS[w].icon}</div><div class="text-[10px] font-bold mt-0.5">${WEAPONS[w].name}</div></button>`).join('')}</div>
    ...
    footer:`<button data-act="new-start" class="tap btn-primary flex-1 py-3 text-base">開始狩獵生涯 ➔</button>`});
}
const readName = () => { const el=$('new-name'); if(el) draft.name=el.value.trim()||'獵人'; };
on('new-icon', async () => { readName(); const v=await modal(`<h2 class="text-lg font-black mb-3">選擇頭像</h2>${iconPickerHtml()}`,[{label:'取消',value:null}],{dismiss:null}); if(v) draft.icon=v; createScreen(); });
...
function hub(){
  ...
  const card=(act,icon,name,desc,badge='')=>`<button data-act="${act}" class="menu-btn relative"><span class="mi">${icon}</span><span class="flex-1 min-w-0"><span class="block font-black">${name}</span><span class="block text-[11px] text-gray-400 truncate">${desc}</span></span>${badge}<span class="text-gray-500">›</span></button>`;
  page({title:'🏕️ 集會所', sub:`${ch.name}・${ch.sub}`, back:'go-menu', right:`<button data-act="go" data-to="settings" data-arg="storyHub" class="tap w-10 text-xl" aria-label="設定">⚙️</button>`, body:`
    <div class="panel p-3 mt-1 flex items-center gap-3">
      <div class="w-14 h-14 rounded-2xl bg-black/40 flex items-center justify-center text-3xl shrink-0">${esc(s.hunter.icon)}</div>
      <div class="flex-1 min-w-0">
        <div class="font-black text-base truncate">${esc(s.hunter.name)}</div>
        <div class="text-[11px] text-gray-400">${w.icon} ${w.name} Lv${wl}（攻擊 ${w.atk+weaponBonus(wl)}）・🦺 防具 Lv${s.armor.lv}（HP ${w.hp+armorBonus(s.armor.lv)}）${s.armor.resist?'・抗'+RESISTS[s.armor.resist]:''}</div>
        <div class="mt-1">${hrBar()}</div>
      </div></div>
    ${nq?`<button data-act="quest" data-id="${nq.id}" class="quest-card ${nq.urgent?'urgent':''} mt-3"><div class="qimg"><img src="${monsterByKey(nq.mons[0]).f}" alt=""></div><div class="flex-1 min-w-0"><div class="text-[10px] text-amber-300 font-bold">建議下一個任務</div><div class="font-black text-sm truncate">${nq.title}</div><div class="text-[10px] text-gray-400">${typeChip(nq)} ${stars(nq)}${nq.turns?'・'+nq.turns+' 回合':''}</div></div><span class="text-gray-500">›</span></button>`:`<div class="panel p-3 mt-3 text-center text-sm text-amber-200">🌅 所有主線任務都完成了！可以回任務板刷素材或挑戰更高評價。</div>`}
    <div class="space-y-3 mt-3">
      ${card('board','📋','任務板',`${CHAPTERS.filter(c=>L.chapterUnlocked(s,c.id)).length}/${CHAPTERS.length} 章節開放・已完成 ${Object.keys(s.cleared).length}/${QUESTS.length}`)}
      ${card('forge','🔨','工房','強化武器與防具、製作卡牌',craft?`<span class="chip bg-amber-500 text-black mr-1">可製作 ${craft}</span>`:'')}
      ${card('box','🎒','道具箱','素材與持有的卡牌')}
      ${card('hunter','🧑','獵人','更換武器、名字與頭像、防具抗性')}
    </div>`});
}
...
function board(){
  ...
  const tabs=CHAPTERS.map(c=>`<button class="${c.id===boardCh?'on':''}" data-act="board-ch" data-ch="${c.id}" ${L.chapterUnlocked(s,c.id)?'':'disabled'}>${L.chapterUnlocked(s,c.id)?'':'🔒'}${c.name}</button>`).join('');
  ...
  const list=questsOf(boardCh).map(q=>{
    ...
    return `<button data-act="quest" data-id="${q.id}" class="quest-card ${q.urgent?'urgent':''} ${ok?'':'locked'}">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-1">${typeChip(q)}<span class="text-[10px] text-gray-400">${stars(q)}</span>${c?`<span class="ml-auto text-[10px] text-green-300">✓ ${c.clears} 次・<span class="rank rank-${c.rank}">${c.rank}</span></span>`:''}</div>
        <div class="font-black text-sm truncate mt-0.5">${ok?'':'🔒 '}${q.title}</div>
        <div class="text-[10px] text-gray-400 truncate">${ok?`${q.turns?'⏳ '+q.turns+' 回合・':''}委託：${q.client}`:L.lockReason(s,q)}</div>
      </div></button>`; }).join('');
  page({title:'📋 任務板', sub:`${ch.name}・${ch.sub}・建議 HR ${ch.hr}`, back:'to-hub', body:`<div class="tabs my-2">${tabs}</div><div class="space-y-2">${list}</div>`});
}
...
function prep(){
  ...
  const monHtml=q.mons.map(k=>{ const m=monsterByKey(k), i=MONSTERS.indexOf(m), sp=L.questMonSpec(q,k), dm=sp.dmgMod;
    return `<button data-act="mon-info" data-i="${i}" class="flex items-center gap-2 panel p-2 flex-1 min-w-0"><div class="w-10 h-10 rounded-lg bg-white p-0.5 shrink-0"><img src="${m.f}" class="w-full h-full object-contain" alt=""></div><div class="min-w-0 text-left"><div class="text-xs font-black truncate">${m.n}</div><div class="text-[10px] text-gray-400">${TIER[m.t].label}・HP ${sp.hp}・攻擊${dm>=0?'+':''}${dm}・看招式 ›</div></div></button>`; }).join('');
  ...
  page({title:q.title, sub:`${CHAPTERS[q.ch].name}・${CHAPTERS[q.ch].sub}`, back:'board', body:`
    <div class="quest-scroll mt-1">
      ...
      <div class="border-t border-amber-900/60 mt-2 pt-2 text-[12px] space-y-0.5">
        <div>🎯 <b>目標</b>：${goal}</div>
        ${q.turns?`<div>⏳ <b>回合上限</b>：${q.turns}　🐱 <b>貓車</b>：3 次</div>`:''}
        <div class="pt-1">🎁 <b>報酬</b>：</div><div class="flex flex-wrap">${dropsHtml(q.reward)}</div>${drops}
        <div class="text-[10px] text-gray-400">HR 經驗 +${c?Math.ceil(q.exp/2)+'（重複）':q.exp}</div>
      </div>
    </div>
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">出場魔物</div>
    <div class="flex gap-2">${monHtml}</div>
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">裝備</div>
    <button data-act="hunter" class="panel p-2 w-full text-left flex items-center gap-2"><span class="text-2xl">${w.icon}</span><div class="flex-1 min-w-0"><div class="text-xs font-black">${w.name} Lv${spec.wlv}・防具 Lv${spec.alv}${spec.resist?'・抗'+RESISTS[spec.resist]:''}</div><div class="text-[10px] text-gray-400">HP ${w.hp+armorBonus(spec.alv)}・攻擊 ${w.atk+weaponBonus(spec.wlv)}・${w.move} 步・技能【${w.skill.name}】</div></div><span class="text-[11px] text-amber-300">更換 ›</span></button>
    <button data-act="deck-edit" class="panel p-2 w-full text-left flex items-center gap-2 mt-2"><span class="text-2xl">🃏</span><div class="flex-1 min-w-0"><div class="text-xs font-black">牌組 ${total} 張・道具 ${items} 張</div><div class="text-[10px] ${err?'text-red-300':'text-gray-400'}">${err||'單人手牌 4 張'}</div></div><span class="text-[11px] text-amber-300">編輯 ›</span></button>`,
    footer: q.type==='tutorial'
      ? `${s.tutorialDone?'':'<button data-act="skip-tutorial" class="tap btn-ghost px-4 text-sm">跳過教學</button>'}<button data-act="depart" class="tap btn-primary flex-1 py-3 text-base">${s.tutorialDone?'重玩訓練':'開始訓練'} ➔</button>`
      : `<button data-act="depart" class="tap btn-primary flex-1 py-3 text-base" ${err?'disabled':''}>${err?'牌組不符規定':'🔥 出發！'}</button>`});
}
...
function deckEdit(){
  ...
  page({title:'🃏 編輯牌組', sub:`${rule.min}–${rule.max} 張・道具最多 ${rule.items} 張・只能放持有的卡`, back:'deck-back', body:`
    ...
    <div class="flex gap-2 my-2"><button data-act="deck-all" class="tap btn-ghost flex-1 text-xs">全部放進去</button><button data-act="forge" class="tap btn-ghost flex-1 text-xs">🔨 去工房做更多卡</button></div>
    ${deckListHtml(s.deck,'story-adj',s.owned,L.hunterSpec(s))}`,
    footer:`<button data-act="deck-back" class="tap btn-primary flex-1 py-3" ${err?'disabled':''}>${err||'完成'}</button>`});
}
...
function forge(){
  ...
  if(forgeTab==='armor'){ const n=L.armorNext(s), ok=n&&L.canAfford(s,n.cost);
    body=`<div class="panel p-3"><div class="flex items-center gap-2"><span class="text-2xl">🦺</span><div class="flex-1"><div class="font-black text-sm">獵人防具 <span class="text-amber-300">Lv${s.armor.lv}</span></div><div class="text-[10px] text-gray-400">最大 HP +${armorBonus(s.armor.lv)}${n?` → Lv${n.lv}：+${armorBonus(n.lv)}${n.lv===ARMOR_MAX_LV?'，並可選一種異常抗性':''}`:'（已滿級）'}</div></div>
      ${n?`<button data-act="craft-armor" class="tap px-3 rounded-lg text-xs font-black ${ok?'btn-primary':'btn-ghost opacity-60'}">強化</button>`:''}</div>
  ...
  if(forgeTab==='card') body=CARD_ORDER.map(id=>{ const c=CARDS_DB[id], r=CARD_RECIPES[id], own=s.owned[id]||0, n=L.cardNext(s,id), ok=n&&L.canAfford(s,n.cost);
    return `<div class="panel p-3 ${own?'':'border-dashed'}"><div class="flex items-center gap-2"><div class="w-9 h-9 rounded-lg flex items-center justify-center text-lg card ${c.type} !p-0" style="flex:0 0 36px">${c.glyph}</div>
      ...
      ${n?`<button data-act="craft-card" data-id="${id}" class="tap px-3 rounded-lg text-xs font-black ${ok?'btn-primary':'btn-ghost opacity-60'}">${own?'製作':'解鎖'}</button>`:`<span class="text-[10px] text-gray-500">${r.cost?'已滿':'基本卡'}</span>`}</div>
      ${n?`<div class="mt-2 flex flex-wrap">${costHtml(n.cost)}</div>`:''}</div>`; }).join('');
  page({title:'🔨 工房', sub:'「素材帶來了？交給我吧！」', back:'forge-back', body:`<div class="tabs my-2">${tabs}</div><div class="space-y-2">${body}</div>`});
}
function resistPicker(){ const s=st();
  return `<div class="panel p-3"><div class="text-sm font-black mb-2">🧿 防具抗性（可隨時更換）</div><div class="seg">${[['','無'],...Object.entries(RESISTS)].map(([k,l])=>`<button class="${(s.armor.resist||'')===k?'on':''}" data-act="set-resist" data-k="${k}">${k?'抗'+l:l}</button>`).join('')}</div></div>`; }
...
function box(){
  ...
  const mon=monIds.length?monIds.map(id=>`<div class="panel p-2 flex items-center gap-2">${matIcon(id,28)}<div class="flex-1 min-w-0 text-[11px] leading-tight">${matName(id)}${MATERIALS[id].kind==='rare'?' <span class="text-amber-300">★</span>':''}</div><b class="text-sm">${L.have(s,id)}</b></div>`).join(''):'<div class="text-xs text-gray-500 col-span-2">還沒有魔物素材。完成任務就會取得。</div>';
  const cards=CARD_ORDER.filter(id=>s.owned[id]>0).map(id=>`<div class="panel p-2 flex items-center gap-2 text-xs"><span class="text-lg">${CARDS_DB[id].glyph}</span><span class="flex-1">${CARDS_DB[id].name}</span><b>${s.owned[id]}</b><span class="text-gray-500 text-[10px]">牌組 ${s.deck[id]||0}</span></div>`).join('');
  page({title:'🎒 道具箱', back:'to-hub', body:`
    <div class="text-[11px] font-bold text-gray-400 mt-2 mb-1">通用素材</div><div class="grid grid-cols-2 gap-1.5">${gen}</div>
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">魔物素材</div><div class="grid grid-cols-2 gap-1.5">${mon}</div>
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">持有卡牌</div><div class="grid grid-cols-2 gap-1.5">${cards}</div>`});
}
...
function hunterPage(){
  here='hunter';
  const s=st();
  page({title:'🧑 獵人', back:'hunter-back', body:`
    <div class="panel p-3 mt-2">
      <div class="flex gap-2">
        ...
        <label class="w-16 shrink-0"><span class="block text-[10px] text-gray-500 mb-0.5">頭像</span>
          <button data-act="hunter-icon" class="w-full bg-black/40 border border-gray-700 rounded-lg py-1.5 text-xl">${esc(s.hunter.icon)}</button></label>
      </div></div>
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">武器（強化等級各自獨立）</div>
    <div class="grid grid-cols-5 gap-1.5">${WEAPON_IDS.map(w=>`<button data-act="hunter-weapon" data-w="${w}" class="wpn-pick ${s.hunter.weapon===w?'sel':''}"><div class="text-xl">${WEAPONS[w].icon}</div><div class="text-[10px] font-bold mt-0.5">${WEAPONS[w].name}</div><div class="text-[9px] text-amber-300">Lv${s.weapons[w]||1}</div></button>`).join('')}</div>
    ...
    <div class="text-[11px] font-bold text-gray-400 mt-3 mb-1">防具 Lv${s.armor.lv}（HP +${armorBonus(s.armor.lv)}）</div>
...
on('hunter-icon', async () => { const v=await modal(`<h2 class="text-lg font-black mb-3">選擇頭像</h2>${iconPickerHtml()}`,[{label:'取消',value:null}],{dismiss:null}); if(v){ st().hunter.icon=v; save(); } hunterPage(); });
...
function questResult(q, r){
  ...
  page({title:'任務結算', sub:q.title, body:`
    <div class="text-center pt-2">
      ...
      <h2 class="text-3xl font-black ${r.win?'text-green-400':'text-red-500'}">${title}</h2>
    ...
    <div class="grid grid-cols-3 gap-2 text-center mt-3">
      <div class="panel p-2"><div class="text-[10px] text-gray-400">回合</div><div class="font-black">${r.turns}${q.turns?`<span class="text-[10px] text-gray-500">/${q.turns}</span>`:''}</div></div>
      <div class="panel p-2"><div class="text-[10px] text-gray-400">貓車</div><div class="font-black">${r.carts}<span class="text-[10px] text-gray-500">/3</span></div></div>
      <div class="panel p-2"><div class="text-[10px] text-gray-400">受到傷害</div><div class="font-black">${r.stats.damageTaken}</div></div>
    </div>
    ${r.win?`<div class="panel p-3 mt-3"><div class="text-[11px] font-bold text-amber-300 mb-1">🎁 獲得素材</div><div class="flex flex-wrap">${dropsHtml(res.drops)}</div></div>
    <div class="panel p-3 mt-2"><div class="text-[11px] font-bold text-amber-300 mb-1">HR 經驗 +${res.exp}${res.firstClear?'':'（重複挑戰減半）'}</div>${hrBar()}${res.hrAfter>res.hrBefore?`<div class="text-center text-amber-300 font-black mt-1">⬆️ HR 提升到 ${res.hrAfter}！</div>`:''}</div>`
    :`<div class="panel p-3 mt-3 text-[11px] text-gray-300 leading-relaxed">💡 提示：${failTip(q,r)}</div>`}
    ${res.unlocked.length?`<div class="space-y-1.5 mt-2">${res.unlocked.map(u=>`<div class="panel p-2 text-sm font-black text-amber-200 border-amber-500/60">${u}</div>`).join('')}</div>`:''}
    <div class="space-y-1.5 mt-2">${achHtml(fresh)}</div>`,
    footer:`<button data-act="to-hub" class="tap btn-ghost px-4">🏕️ 集會所</button>
      ${q.type==='tutorial'?'':`<button data-act="retry" class="tap btn-ghost px-4">🔁 再挑戰</button>`}
      ${nq&&r.win?`<button data-act="quest" data-id="${nq.id}" class="tap btn-primary flex-1 py-3 text-sm">下一個任務 ➔</button>`:`<button data-act="retry-prep" class="tap btn-primary flex-1 py-3 text-sm">${r.win?'任務板':'調整後再戰'}</button>`}`});
}
